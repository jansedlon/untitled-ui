...
export const textInputStyles = tv({
	slots: {
		...
		input:
			"flex-grow-1 z-[1] h-10 w-full rounded-lg border px-3 py-2 shadow-sm focus:outline-none focus:ring-4 focus:ring-slate-100 transition disabled:pointer-events-none disabled:select-none disabled:bg-slate-50 disabled:text-slate-500 sm:text-sm sm:leading-6 [&[data-with-end-adornment='true']]:rounded-r-none [&[data-with-icon='true']]:pl-8",
		endAdornment:
			"z-0 flex h-10 flex-grow-0 items-center justify-center rounded-lg rounded-l-none shadow-sm [&>*]:h-full [&>*]:rounded-l-none",
		rightSection:
